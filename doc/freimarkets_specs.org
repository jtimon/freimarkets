...
New-style outputs contain the =decimal64= continuous value combined with a possibly empty list of bitstrings. These bitstrings are indivisible, unique outputs. Any output unique must be found in an input of the enclosing transaction, and uniques cannot be shared among two outputs of the same transaction.

The asset definition transaction, or any transaction with one or more of an assets’ issuers as signatory is allowed to violate the constraint that continuous outputs are less than or equal to inputs, and that output uniques are a subset of inputs for that asset.
...
Fractional redemption of outputs containing uniques is not allowed.
...
New-style transactions have a sorted list of ~<assetid:bitstring, scriptSig>~ signatories. The =assetid= is the 20-byte asset tag, with bitstring taking the remaining bytes. The =scriptPubKey= is retrieved from the unspent transaction output containing the identified unique.
...
- Outputs are suffixed with an optionally empty, sorted.

- A sorted-list mapping of ~<assetid:unique, scriptSig>~ signatories is added immediately following =vout=.
