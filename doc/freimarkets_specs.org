...
5. Each signature in the signatories mapping must reference an existing token, execute correctly, and finish execution with a non-zero value as the top item on the stack or else the transaction does not validate.
