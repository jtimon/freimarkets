...
** REVIEW [2/2] New scripting opcodes
...
#+BEGIN_SRC bitcoin
<tx_id> <output number> <chain_id> <height> OP_OUT_SPENT
#+END_SRC
...
2) The output identified by <tx_id>:<output number> still exists in the UTXO of <chain_id> at block height <height>.

Returns 1 otherwise.

*** REVIEW =OUT_EXISTS=
#+BEGIN_SRC bitcoin
<amount> <scriptPubKeyHash> <asset_id> <chain_id> <from> <to> OP_OUT_EXISTS
#+END_SRC
...
2) There's no output in <chain_id> from block height <from> to block
   height <to> that matches the <asset_id>, <amount> and a
...
First the sub-transactions that contains all these terms is created
(but not signed).

Option sub-transaction:

#+BEGIN_SRC bitcoin
input: Q pubA (script1)
output: Q * X pubB to <seller1 pkh>
expiry: Exp
#+END_SRC

The output script in the input (script1) uses p2sh and looks like this:

#+BEGIN_SRC bitcoin
if (block_time < exp)
   <buyer1 pkh> OP_EQUALVERIFY OP_CHECKSIGVERIFY
else
   <seller2 pkh> OP_EQUALVERIFY OP_CHECKSIGVERIFY
#+END_SRC

The buyer signs the following sub-transaction:

#+BEGIN_SRC bitcoin
input: P pubB
output: Q pubA to <script1 hash>
expiry: Exp2
#+END_SRC

The seller broadcast this last sub-transactions after completing it with:

#+BEGIN_SRC bitcoin
input: Q pubA
output: P pubB to <seller3 pkh>
#+END_SRC

When this transaction gets into the chain, the contract is complete,
the premium payment to the seller is final and the buyer can securely
exercise his right to buy pubA with pubB at the specified price,
because the seller cannot double-spend the funds in script1 until
Exp, which cancels the option subtransaction.

Exp2 must be earlier than Exp to protect the seller from DoS attacks
from fake buyers.

The payer could at any moment before Exp complete the option sub-transaction with:
...
In fact, the option subtransaction could have granularity allowing
